...
type LegacyRightSidebarState = Partial<RightSidebarState> & { open?: boolean };
...
function normalizeRightSidebarState(value: unknown): RightSidebarState {
  ...
  const data = value as Partial<RightSidebarState>;
  const legacyOpen = 'open' in data && typeof (data as { open?: unknown }).open === 'boolean'
    ? (data as { open: boolean }).open
    : undefined;
