...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!oauthState?.ownerId) {
      sendGoogleCallbackHtml(res, APP_ID, false, 'OAuth state expired. Please reconnect from LuGmail.');
      return;
    }
    ...
    sendGoogleCallbackHtml(res, APP_ID, true, `Connected ${email} to LuGmail.`);
  } catch (callbackError) {
    res.statusCode = 500;
    sendGoogleCallbackHtml(
      res,
      APP_ID,
      false,
      callbackError instanceof Error ? callbackError.message : 'Unable to connect LuGmail.',
    );
  }
}
