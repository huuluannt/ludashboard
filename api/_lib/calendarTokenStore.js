...
const TOKEN_PREFIX = process.env.LUCALENDAR_REDIS_PREFIX || 'ludashboard:lucalendar';
...
export async function readCalendarTokenStore() {
  throw new Error('LuCalendar token persistence uses Upstash Redis and does not support filesystem reads.');
}
...
function redis() {
  ...
  if (!url || !token) {
    throw new Error(
      'LuCalendar requires KV_REST_API_URL and KV_REST_API_TOKEN, or UPSTASH_REDIS_REST_URL and UPSTASH_REDIS_REST_TOKEN, for server-side token storage.',
    );
  }
...
function encryptionKey() {
  const source =
    ...
    '';
  if (!source) {
    throw new Error('LUCALENDAR_TOKEN_SECRET or Upstash Redis token is required to protect LuCalendar OAuth tokens.');
  }
